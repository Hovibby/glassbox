...
//   - Environment variable: GLASSBOX_NO_UPDATE_CHECK
...
func TestConfigPrecedenceIntegration(t *testing.T) {
	...
	origErstNoUpdate := os.Getenv("GLASSBOX_NO_UPDATE_CHECK")

	t.Cleanup(func() {
		_ = os.Setenv("HOME", origHome)
		_ = os.Setenv("XDG_CONFIG_HOME", origXDGConfigHome)
		_ = os.Setenv("AppData", origAppData)
		_ = os.Setenv("GLASSBOX_NO_UPDATE_CHECK", origErstNoUpdate)
	})
	...
	t.Run("config disables updates when env is unset", func(t *testing.T) {
		require.NoError(t, os.Unsetenv("GLASSBOX_NO_UPDATE_CHECK"))
	...
	t.Run("environment variable takes precedence over config", func(t *testing.T) {
		...
		require.NoError(t, os.Setenv("GLASSBOX_NO_UPDATE_CHECK", "1"))

		checker := NewChecker("v1.0.0")
		disabled := checker.isUpdateCheckDisabled()
		assert.True(t, disabled, "GLASSBOX_NO_UPDATE_CHECK should disable updates even if config enables them")
	})
}
