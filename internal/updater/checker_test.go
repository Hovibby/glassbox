...
func TestOptOut(t *testing.T) {
	...
	t.Run("GLASSBOX_NO_UPDATE_CHECK=1 disables checker", func(t *testing.T) {
		_ = os.Setenv("GLASSBOX_NO_UPDATE_CHECK", "1")
		defer func() { _ = os.Unsetenv("GLASSBOX_NO_UPDATE_CHECK") }()

		assert.True(t, checker.isUpdateCheckDisabled())
	})

	t.Run("unset GLASSBOX_NO_UPDATE_CHECK enables checker", func(t *testing.T) {
		_ = os.Unsetenv("GLASSBOX_NO_UPDATE_CHECK")
...
func TestGetCacheDir(t *testing.T) {
	...
	assert.Contains(t, cacheDir, "glassbox")
}
...
func TestAPIIntegration(t *testing.T) {
	t.Run("successful API response", func(t *testing.T) {
		server := httptest.NewServer(http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
			assert.Equal(t, "Glassbox-cli", r.Header.Get("User-Agent"))
