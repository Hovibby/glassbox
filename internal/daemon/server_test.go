...
func TestServer_DebugTransaction(t *testing.T) {
	...
	t.Setenv("GLASSBOX_SIM_PATH", simPath)

	server, err := NewServer(Config{
		Network: string(stellarrpc.Testnet),
	})
	if err != nil {
		// Skip if simulator binary not found (expected in CI without glassbox-sim)
		if strings.Contains(err.Error(), "glassbox-sim binary not found") {
			t.Skip("Skipping test: glassbox-sim binary not found")
		}
...
func TestServer_GetContractCode(t *testing.T) {
	t.Setenv("GLASSBOX_SIM_PATH", os.Args[0])
