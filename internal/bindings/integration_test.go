...
func TestGenerateBindings(t *testing.T) {
	...
	t.Run("GenerateClient", func(t *testing.T) {
		...
		// Check for Glassbox integration
		if !strings.Contains(client, "ErstSimulator") {
			t.Error("Missing Glassbox integration")
		}
	})

	// Test Glassbox integration generation
	t.Run("GenerateErstIntegration", func(t *testing.T) {
		Glassbox := generator.generateErstIntegration()

		// Check for simulator class
		if !strings.Contains(Glassbox, "export class ErstSimulator") {
			t.Error("Missing ErstSimulator class")
		}

		// Check for simulate method
		if !strings.Contains(Glassbox, "async simulate(") {
			t.Error("Missing simulate method")
		}

		// Check for debug method
		if !strings.Contains(Glassbox, "async debugTransaction(") {
