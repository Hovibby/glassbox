...
Example workflow:
  ...
  Glassbox offline generate --network testnet --source GABC... -o tx.Glassbox.json ./tx.xdr

  # Copy tx.Glassbox.json to USB → bring to air-gapped machine
  Glassbox offline sign --key <hex-seed> tx.Glassbox.json

  # Copy signed tx.Glassbox.json back via USB
  Glassbox offline verify tx.Glassbox.json
  Glassbox offline submit tx.Glassbox.json`,
}
...
machine for signing.`,
	Example: `  Glassbox offline generate --network testnet -o tx.Glassbox.json ./tx.xdr
  Glassbox offline generate --network mainnet --source GABC... -o unsigned.json ./tx.xdr`,
...
func runOfflineGenerate(_ *cobra.Command, args []string) error {
	...
	fmt.Printf("  Glassbox offline sign --key <hex-seed> %s\n", output)
...
var offlineSignCmd = &cobra.Command{
	Use:   "sign <envelope.Glassbox.json>",
	Short: "Sign an envelope file with a secret key",
	Long: `Load a portable envelope file generated by 'Glassbox offline generate',
...
It can be passed via --key flag or the GLASSBOX_SIGN_KEY environment variable.`,
	Example: `  Glassbox offline sign --key <hex-seed> tx.Glassbox.json
  GLASSBOX_SIGN_KEY=<hex-seed> Glassbox offline sign tx.Glassbox.json`,
	Args: cobra.ExactArgs(1),
	RunE: runOfflineSign,
}
...
func runOfflineSign(_ *cobra.Command, args []string) error {
	...
	key := offlineKeyFlag
	if key == "" {
		key = os.Getenv("GLASSBOX_SIGN_KEY")
	}
	if key == "" {
		return errors.WrapValidationError("private key is required (use --key or GLASSBOX_SIGN_KEY env var)")
	}
	...
	fmt.Printf("  File: %s\n", path)
	fmt.Println("\nNext steps:")
	fmt.Printf("  Glassbox offline verify %s\n", path)
	fmt.Printf("  Glassbox offline submit %s\n", path)
...
var offlineSubmitCmd = &cobra.Command{
	Use:   "submit <envelope.Glassbox.json>",
...
network via the Soroban RPC sendTransaction endpoint.`,
	Example: `  Glassbox offline submit tx.Glassbox.json
  Glassbox offline submit --rpc-url https://custom-rpc.example.com tx.Glassbox.json`,
	Args: cobra.ExactArgs(1),
...
func runOfflineSubmit(cmd *cobra.Command, args []string) error {
	...
	if !ef.IsSigned() {
		return errors.WrapValidationError("envelope has no signatures; sign it first with 'Glassbox offline sign'")
	}
