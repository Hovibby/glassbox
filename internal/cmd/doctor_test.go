...
func TestCheckSimulator(t *testing.T) {
	...
	if dep.Name != "Simulator Binary (glassbox-sim)" {
		t.Errorf("checkSimulator() name = %v, want 'Simulator Binary (glassbox-sim)'", dep.Name)
	}
...
func TestCheckSimulatorPaths(t *testing.T) {
	...
	// 1. PATH environment
	// 2. simulator/target/release/glassbox-sim
	// 3. ./glassbox-sim
	// 4. ../simulator/target/release/glassbox-sim
...
func TestCheckConfigTOML(t *testing.T) {
	...
	// valid config
	_ = os.WriteFile(".Glassbox.toml", []byte("rpc_url = \"https://example.com\"\n"), 0644)
	dep = checkConfigTOML(false)
	if !dep.Installed {
	...
	// invalid syntax
	_ = os.WriteFile(".Glassbox.toml", []byte("rpc_url = \n"), 0644)
...
func TestCheckRPC(t *testing.T) {
	...
	defer rs.Close()
	os.Setenv("GLASSBOX_RPC_URL", rs.URL())
	...
	// bad url
	os.Setenv("GLASSBOX_RPC_URL", "http://nonexistent.invalid")
	dep = checkRPC(false)
	if dep.Installed {
...
func TestCheckCacheDir(t *testing.T) {
	...
	if dep.Name != "Cache directory (~/.Glassbox)" {
		t.Errorf("checkCacheDir() name = %v, want 'Cache directory (~/.Glassbox)'", dep.Name)
	}
...
func TestCheckDeepLink_Name(t *testing.T) {
	...
	if dep.Name != "Deep link (glassbox:// scheme)" {
		t.Errorf("checkDeepLink() name = %q, want %q", dep.Name, "Deep link (glassbox:// scheme)")
	}
}
