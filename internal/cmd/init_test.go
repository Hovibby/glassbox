...
func TestScaffoldErstProjectCreatesFilesAndDirs(t *testing.T) {
	...
	erstToml, err := os.ReadFile(filepath.Join(root, "Glassbox.toml"))
	require.NoError(t, err)
	...
	assert.Contains(t, string(erstToml), `cache_path = ".glassbox/cache"`)

	gitignore, err := os.ReadFile(filepath.Join(root, ".gitignore"))
	require.NoError(t, err)
	assert.Contains(t, string(gitignore), "# Glassbox local debugging artifacts")
	assert.Contains(t, string(gitignore), ".glassbox/traces/")
}

func TestScaffoldErstProjectDoesNotOverwriteErstTomlWithoutForce(t *testing.T) {
	root := t.TempDir()
	path := filepath.Join(root, "Glassbox.toml")
	require.NoError(t, os.WriteFile(path, []byte("network = \"public\"\n"), 0644))

	err := scaffoldErstProject(root, initScaffoldOptions{Network: "testnet"})
	require.Error(t, err)
	assert.Contains(t, err.Error(), "Glassbox.toml already exists")
...
func TestEnsureGitignoreBlockIsIdempotent(t *testing.T) {
	...
	assert.Equal(t, 1, strings.Count(text, "# Glassbox local debugging artifacts"))
}
