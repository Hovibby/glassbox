...
func TestCheckProtocolDarwinWithValidPlist(t *testing.T) {
	...
	tmpDir := t.TempDir()
	plistPath := filepath.Join(tmpDir, "com.glassbox.protocol.plist")
	content := `<?xml version="1.0" encoding="UTF-8"?>
<plist version="1.0">
<dict>
    <key>CFBundleURLSchemes</key>
    <array><string>Glassbox</string></array>
</dict>
</plist>`
	...
	if !strings.Contains(string(data), "<string>Glassbox</string>") {
		t.Error("test plist should contain Glassbox scheme")
	}
}
...
func TestRepairProtocolDarwin(t *testing.T) {
	...
	err := repairProtocolDarwin("/usr/local/bin/Glassbox", &buf)
	// launchctl load may fail in test env, but the file should be written
	plistPath := filepath.Join(tmpDir, "Library", "LaunchAgents", "com.glassbox.protocol.plist")
	data, readErr := os.ReadFile(plistPath)
	if readErr != nil {
		t.Fatalf("plist file was not written: %v (repair err: %v)", readErr, err)
	}
	if !strings.Contains(string(data), "<string>Glassbox</string>") {
		t.Error("plist missing Glassbox scheme")
	}
	if !strings.Contains(string(data), "/usr/local/bin/Glassbox") {
...
func TestRepairProtocolLinux(t *testing.T) {
	...
	_ = repairProtocolLinux("/usr/local/bin/Glassbox", &buf)

	desktopPath := filepath.Join(tmpDir, ".local", "share", "applications", "glassbox-protocol.desktop")
	data, err := os.ReadFile(desktopPath)
	if err != nil {
		t.Fatalf("desktop file was not written: %v", err)
	}
	if !strings.Contains(string(data), "x-scheme-handler/Glassbox") {
		t.Error("desktop file missing Glassbox scheme handler")
	}
	if !strings.Contains(string(data), "/usr/local/bin/Glassbox") {
...
func TestStatusOutputRegistered(t *testing.T) {
	...
	output := buf.String()
	if !strings.Contains(output, "GLASSBOX Protocol Registration Status") {
