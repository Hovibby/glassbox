...
func checkRegistration(selfPath string) Result {
	...
	out, err := exec.Command("bash", "-c",
		`/System/Library/Frameworks/CoreServices.framework/Frameworks/LaunchServices.framework/Support/lsregister -dump 2>/dev/null | grep -i "glassbox://" | head -5`,
	).Output()

	if err == nil && strings.Contains(strings.ToLower(string(out)), "glassbox") {
		res.Registered = true
	...
	// Fallback: try `open -Ra glassbox://` which exits 0 when a handler exists.
