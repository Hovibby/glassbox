...
func TestSend_NoOpWhenDisabled(t *testing.T) {
	...
	err := reporter.Send(context.Background(), errors.New("test"), nil, "Glassbox debug")
...
func TestSend_PostsPayloadWhenEnabled(t *testing.T) {
	...
	assert.Equal(t, "Glassbox debug", received.Command)
...
func TestSend_UserAgentIncludesVersion(t *testing.T) {
	...
	assert.Equal(t, "Glassbox/0.9.0", gotUA)
}
...
func TestBuildReport_FieldsPopulated(t *testing.T) {
	...
	report := reporter.buildReport(errors.New("something failed"), []byte("stack"), "Glassbox session list")
	...
	assert.Equal(t, "Glassbox session list", report.Command)
...
func TestHandlePanic_NoOpOnNoPanic(t *testing.T) {
	...
	assert.NotPanics(t, func() {
		reporter.HandlePanic(context.Background(), "Glassbox debug")
	})
}
...
func TestHandlePanic_ReportsAndRepanicsOnError(t *testing.T) {
	...
	assert.Panics(t, func() {
		defer reporter.HandlePanic(context.Background(), "Glassbox debug")
...
// TestNew_SentryDSNFromEnv verifies that GLASSBOX_SENTRY_DSN is picked up by New
