...
// Package crashreport provides opt-in anonymous crash reporting for the Glassbox CLI.
//
// Two independent sinks are supported and may be used together:
//
//   - Sentry: supply a DSN via SentryDSN in Config or the GLASSBOX_SENTRY_DSN
//     environment variable.  The official Sentry Go SDK is used.
//
//   - Custom endpoint: supply an HTTPS URL via Endpoint in Config or the
//     GLASSBOX_CRASH_ENDPOINT environment variable.  A JSON Report is POSTed.
//
// Both sinks are disabled by default.  Users must explicitly opt in via the
// config file (crash_reporting = true) or the GLASSBOX_CRASH_REPORTING environment
// variable.  No personal data or transaction content is ever collected: only
// the error message, stack trace, OS/arch, Go version, and Glassbox version.
package crashreport
...
const (
	...
	DefaultEndpoint = "https://crash.Glassbox.dev/v1/report"
...
type Report struct {
	...
	// Command is the cobra command path that was executing (e.g. "Glassbox debug").
...
type Config struct {
	...
	// forwarded to Sentry using the official Go SDK.  The GLASSBOX_SENTRY_DSN
	...
	// The GLASSBOX_CRASH_ENDPOINT environment variable overrides this value at
	// runtime.
	Endpoint string
...
//
//	GLASSBOX_SENTRY_DSN        overrides cfg.SentryDSN
//	GLASSBOX_CRASH_ENDPOINT    overrides cfg.Endpoint
//	GLASSBOX_CRASH_REPORTING   overrides cfg.Enabled (checked at send time)
//
...
// GLASSBOX_CRASH_REPORTING=true still gets a working reporter.
...
// The GLASSBOX_CRASH_REPORTING environment variable takes precedence over the
...
func (r *Reporter) sendToEndpoint(ctx context.Context, report Report) error {
	...
	req.Header.Set("User-Agent", "Glassbox/"+r.cfg.Version)
