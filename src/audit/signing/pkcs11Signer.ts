...
export const resolvePkcs11KeyIdHex = (cfg: { keyIdHex?: string; pivSlot?: string }): string | undefined => {
  if (cfg.keyIdHex) {
    ...
    if (!/^[0-9a-fA-F]+$/.test(normalized) || normalized.length % 2 !== 0) {
      throw new Error(`Invalid GLASSBOX_PKCS11_KEY_ID '${cfg.keyIdHex}'. Expected an even-length hex string.`);
    }
    return normalized;
  }
...
const resolvePkcs11Slot = (opts: {
  ...
  if (opts.tokenLabel) {
    ...
    throw new Error(`GLASSBOX_PKCS11_TOKEN_LABEL (${opts.tokenLabel}) did not match any tokens.`);
  }
...
export class Pkcs11Signer implements AuditSigner {
  private readonly cfg = {
    ...
    algorithm: (process.env.GLASSBOX_PKCS11_ALGORITHM || 'ed25519').toLowerCase(),
  };
  ...
  async public_key(): Promise<PublicKey> {
    ...
    throw new Error('Set GLASSBOX_PKCS11_PUBLIC_KEY_PEM to a SPKI PEM public key.');
  }
