...
func TestProtocolRegistrationLifecycle(t *testing.T) {
	if os.Getenv("GLASSBOX_RUN_PROTOCOL_REGISTRATION_TESTS") != "1" {
		t.Skip("set GLASSBOX_RUN_PROTOCOL_REGISTRATION_TESTS=1 to enable protocol registration integration tests")
	}
	...
	assertContains(t, "register output", stdout+stderr, "Registered GLASSBOX Protocol handler")

	stdout, stderr, err = runErst(t, "protocol:verify")
	assertExitCode(t, 0, err)
	assertContains(t, "verify output", stdout+stderr, "Verified GLASSBOX Protocol registration")

	scriptPath := os.Getenv("GLASSBOX_PROTOCOL_VERIFY_SCRIPT")
	if scriptPath == "" {
		t.Fatal("GLASSBOX_PROTOCOL_VERIFY_SCRIPT must be set when protocol registration tests are enabled")
	}
	...
	assertExitCode(t, 0, err)
	assertContains(t, "unregister output", stdout+stderr, "Unregistered GLASSBOX Protocol handler")
...
func runNativeVerificationScript(t *testing.T, scriptPath string) {
	...
	cmd.Env = append(os.Environ(), "GLASSBOX_BINARY="+binaryPath(t))
