...
-----END PUBLIC KEY-----`;
  ...
  beforeEach(() => {
    process.env.GLASSBOX_KMS_REGION = 'us-east-1';
    process.env.GLASSBOX_KMS_KEY_ID = testKeyId;
    process.env.GLASSBOX_KMS_PUBLIC_KEY_PEM = testPublicKeyPem;
  });

  afterEach(() => {
    delete process.env.GLASSBOX_KMS_REGION;
    delete process.env.GLASSBOX_KMS_KEY_ID;
    delete process.env.GLASSBOX_KMS_PUBLIC_KEY_PEM;
  });

  test('constructor validates required environment variables', () => {
    delete process.env.GLASSBOX_KMS_KEY_ID;
    expect(() => new KmsEd25519Signer()).toThrow('GLASSBOX_KMS_KEY_ID is not set');
  });

  test('constructor validates public key environment variable', () => {
    delete process.env.GLASSBOX_KMS_PUBLIC_KEY_PEM;
    expect(() => new KmsEd25519Signer()).toThrow('GLASSBOX_KMS_PUBLIC_KEY_PEM is not set');
  });
  ...
  test('uses configured region or defaults to us-east-1', () => {
    process.env.GLASSBOX_KMS_REGION = 'eu-west-1';
    const signer = new KmsEd25519Signer();
