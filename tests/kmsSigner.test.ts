...
function setKmsEnv() {
  process.env.GLASSBOX_KMS_KEY_ID = 'arn:aws:kms:us-east-1:123456789012:key/test-key-id';
...
describe('KmsSigner', () => {
  ...
  describe('constructor', () => {
    it('throws when GLASSBOX_KMS_KEY_ID is missing', () => {
      delete process.env.GLASSBOX_KMS_KEY_ID;
    ...
    it('uses GLASSBOX_KMS_SIGNING_ALGORITHM env var when set', () => {
      process.env.GLASSBOX_KMS_SIGNING_ALGORITHM = 'RSASSA_PSS_SHA_256';
      const signer = new KmsSigner();
  ...
  describe('sign()', () => {
    ...
    it('calls KMS SignCommand with correct parameters', async () => {
      ...
      expect(mock.SignCommand).toHaveBeenCalledWith({
        KeyId: process.env.GLASSBOX_KMS_KEY_ID,
        Message: digest,
        MessageType: 'DIGEST',
    ...
    it('passes the configured signing algorithm to KMS', async () => {
      process.env.GLASSBOX_KMS_SIGNING_ALGORITHM = 'ECDSA_SHA_512';
      const mock = buildKmsMock();
      injectKmsMock(mock);
