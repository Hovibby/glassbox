...
printf '// Copyright (c) glassbox Authors.\n// SPDX-License-Identifier: Apache-2.0\n\npackage main\n' > "$T/main.go"
...
assert_file_contains "Fix mode prepends header to Go file" "$T/fixme.go" "// Copyright (c) glassbox Authors."
rm -rf "$T"
...
add_tracked "$T" lib.rs
run_audit "$T" > /dev/null 2>&1 || true
assert_file_contains "Fix mode prepends header to Rust file" "$T/lib.rs" "// Copyright (c) glassbox Authors."
rm -rf "$T"
