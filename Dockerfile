...
RUN go build -ldflags="-s -w" -o Glassbox cmd/Glassbox/main.go
...
COPY --from=builder-go /app/Glassbox .
COPY --from=builder-rust /app/simulator/target/release/glassbox-sim ./simulator/target/release/glassbox-sim

# Verify binaries are executable
RUN chmod +x ./Glassbox ./simulator/target/release/glassbox-sim

# Add health check
HEALTHCHECK --interval=30s --timeout=3s --start-period=5s --retries=3 \
  CMD ./Glassbox --version || exit 1
